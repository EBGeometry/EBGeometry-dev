...
namespace EBGeometry::MeshParser {
  ...
  turnPolygonSoupIntoDCEL(PolygonSoup<MetaData>& a_soup) noexcept
  {
    ...
    auto vertices = new Vertex<MetaData>[numVertices];
    auto edges    = new Edge<MetaData>[numEdges];
    auto faces    = new Face<MetaData>[numFaces];
    ...
    for (const auto& m : outgoingEdgesMap) {
      ...
      for (const auto& curOutgoingEdgeIndex : outgoingEdges) {
        ...
        for (const auto& otherOutgoingEdgeIndex : outgoingEdges) {
